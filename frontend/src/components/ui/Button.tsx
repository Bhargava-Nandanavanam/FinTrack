...
export default function Button({ variant="primary", asChild, className, href, ...props }: Props) {
  const base =
    "inline-flex items-center justify-center rounded-md px-5 py-2.5 text-sm font-medium transition-colors focus-visible:ring-2 focus-visible:ring-[--color-ring]";
  const styles = {
  ...
  if (asChild && href) return <Link href={href} className={classes}>{props.children}</Link>;
